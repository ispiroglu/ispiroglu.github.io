# Commands
- **Build**: `bun run build` (generate-stack → `astro build`)
...
- MDX content system: posts in `content/writing/`, stack in `content/stack/`, auto-generated to `app/lib/stack.generated.ts`; post bodies render straight from MDX via the `writing` content collection (`app/content.config.ts`) and `@astrojs/mdx`
