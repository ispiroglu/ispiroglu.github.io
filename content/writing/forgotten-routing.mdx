...
Late one evening, you just got home, changed, and picked up your phone to look for a piece of clothing you had been wanting. But the seller's page didn't quite explain the material, the size table, whether it was cotton or polyester, whether it would fit you.
...
Elasticsearch did not fall. The cluster still had room left. What throttled us in the end was Java CPU on the applications. _That is whole another problem of ours_

## Looking past the one-line fix

**_The Forgotten Routing Key_**, restated in one breath: a term in the query body filters, and only `_routing` points. Two jobs sat on one page. We pointed the list and left the count waking twelve shards. In other words, a heavy query that runs on all shards instead just one and it caused an exponential increase in CPU usage.
...
**In conclusion**, the line was already in the house. The rewrite was not required. **We must control our urge to produce more code.**. We still own the system and the walk is the reason. When a bigger system feels close, the query we already fear is still the first place to look.
